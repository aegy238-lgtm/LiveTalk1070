...
const VoiceRoom: React.FC<any> = ({ 
  ...
  onOpenCP, giftCategoryLabels, isMinimized, onPushState
}) => {
  ...
  useEffect(() => {
    ...
    const q = query(collection(db, 'rooms', initialRoom.id, 'messages'), where('timestamp', '>=', joinTime), orderBy('timestamp', 'asc'), limit(50));
  ...
  useEffect(() => {
    const unsubSkins = onSnapshot(doc(db, 'appSettings', 'micSkins'), (snap) => {
      if (snap.exists()) setMicSkins(snap.data() as Record<string, string>);
    });
    return () => unsubSkins();
  }, []);
  ...
  useEffect(() => {
    ...
    setDoc(participantRef, { id: currentUser.id, name: currentUser.name, avatar: currentUser.avatar, customId: currentUser.customId || null, wealthLevel: currentUser.wealthLevel || 1, isVip: currentUser.isVip || false, badge: currentUser.badge || null, joinedAt: serverTimestamp() });
    return () => { 
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      if (isHost) updateDoc(doc(db, 'rooms', initialRoom.id), { listeners: parts.length }).catch(() => {});
    });
    return () => unsubscribe();
  ...
  useEffect(() => {
    ...
    return () => unsubRoom();
  }, [initialRoom.id, currentUser.id, onLeave]);
  ...
  const handleSeatClick = (index: number) => {
    ...
    if (s) { 
      onPushState?.(); 
      setSelectedUserForProfile(s); 
      setShowProfileSheet(true); 
    }
    else {
      if (room.kickedUsers?.includes(currentUser.id)) return;
      const newSpeaker = { id: currentUser.id, customId: currentUser.customId, name: currentUser.name, avatar: currentUser.avatar, seatIndex: index, isMuted, charm: (localSpeakers.find(s => s.id === currentUser.id)?.charm || 0), activeEmoji: null, frame: currentUser.frame || null };
  ...
  const handleSendGift = (gift: Gift, quantity: number) => {
    ...
    if (executeGiftSendOptimistic(gift, quantity, selectedRecipientIds, false)) {
      ...
      comboExpireTimerRef.current = setTimeout(() => { setComboState(null); }, 5000); 
    }
  };
  ...
  const executeGiftSendOptimistic = (gift: Gift, quantity: number, recipientIds: string[], isComboHit: boolean = false) => {
    ...
    // تشغيل الأنميشن والصوت محلياً فوراً
    ...
    if (gift.isLucky || gift.category === 'lucky') {
      ...
      if (isWin && gameSettings.luckyMultipliers?.length > 0) {
        const totalChance = gameSettings.luckyMultipliers.reduce((sum, m) => sum + m.chance, 0);
        let random = Math.random() * totalChance;
        let picked = gameSettings.luckyMultipliers[0];
        for (const m of gameSettings.luckyMultipliers) { if (random < m.chance) { picked = m; break; } random -= m.chance; }
        winAmount = gift.cost * quantity * picked.value;
      }
    }

    // 🔥 تحديث رصيد المستخدم فوراً في الواجهة (Optimistic UI)
    onUpdateUser({ 
    ...
    // التنسيق في الخلفية مع قاعدة البيانات
    if (isComboHit) {
      ...
      comboSyncTimerRef.current = setTimeout(() => {
  ...
  const handleSendLuckyBag = async (amount: number, recipients: number) => {
    if (Number(currentUser.coins || 0) < amount) return alert('رصيدك لا يكفي');
    const bagId = 'bag_' + Date.now();
    const expiresAt = new Date(Date.now() + 5 * 60 * 1000); 
    try {
      const batch = writeBatch(db);
      batch.set(doc(db, 'lucky_bags', bagId), { id: bagId, senderId: currentUser.id, senderName: currentUser.name, senderAvatar: currentUser.avatar, roomId: initialRoom.id, roomTitle: initialRoom.title, totalAmount: amount, remainingAmount: amount, recipientsLimit: recipients, claimedBy: [], createdAt: serverTimestamp(), expiresAt: Timestamp.fromDate(expiresAt) });
      ...
      batch.set(messageRef, { userName: 'النظام', content: `${currentUser.name} أطلق حقيبة حظ بمبلغ ${amount.toLocaleString()} 🪙!`, type: 'system', timestamp: serverTimestamp() });
      await batch.commit();
  ...
  const handleSendMessage = (text: string) => {
    ...
    addDoc(collection(db, 'rooms', initialRoom.id, 'messages'), { 
        userId: currentUser.id, userName: currentUser.name, userWealthLevel: calculateLiveLvl(Number(currentUser.wealth || 0)), 
        userRechargeLevel: calculateLiveLvl(Number(currentUser.rechargePoints || 0)), userAchievements: currentUser.achievements || [], 
        userBubble: currentUser.activeBubble || null, userVip: currentUser.isVip || false, content: text, type: 'text', timestamp: serverTimestamp() 
    });
  };
  ...
  return (
    <div className="fixed inset-0 z-[150] flex flex-col bg-slate-950 font-cairo overflow-hidden text-right">
      ...
      <RoomHeader 
        ...
        onShowParticipants={() => { onPushState?.(); setShowParticipants(true); }} 
      />
      <AnimatePresence>
      ...
      <div className="flex-1 relative flex flex-col overflow-hidden">
        ...
        <div className="h-64 px-4 mb-4 overflow-hidden relative z-[10]" dir="rtl">
           <div ref={chatContainerRef} className="h-full overflow-y-auto scrollbar-hide space-y-4 flex flex-col pb-4 pointer-events-auto touch-pan-y">
              ...
              {messages.map((msg) => (
                <motion.div key={msg.id || Math.random()} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="flex items-start gap-2">
        ...
        <AnimatePresence>
          {comboState && (
            <ComboButton gift={comboState.gift} count={comboState.count} onHit={() => { 
        ...
        <ControlBar isMuted={isMuted} onToggleMute={onToggleMute} onShowGifts={() => { onPushState?.(); setShowGifts(true); }} onShowGames={() => { onPushState?.(); setShowGameCenter(true); }} onShowRoomTools={() => { onPushState?.(); setShowTools(true); }} onSendMessage={handleSendMessage} onShowEmojis={() => { onPushState?.(); setShowEmojis(true); }} userCoins={Number(currentUser.coins)} />
      </div>
      ...
      <RoomToolsModal isOpen={showTools} onClose={() => setShowTools(false)} isHost={canAdmin} onAction={(action) => {
          if (action === 'settings') { onPushState?.(); setShowSettings(true); }
          else if (action === 'rank') { onPushState?.(); setShowRank(true); }
          else if (action === 'luckybag') { onPushState?.(); setShowLuckyBag(true); }
            ...
            getDocs(collection(db, 'rooms', initialRoom.id, 'messages')).then(snap => { const batch = writeBatch(db); snap.forEach(d => batch.delete(d.ref)); batch.commit(); });
          } else if (action === 'reset_charm') {
            if (!confirm('تصفير الكاريزما؟')) return;
      ...
      <GameCenterModal isOpen={showGameCenter} onClose={() => setShowGameCenter(false)} onSelectGame={(game) => { onPushState?.(); setActiveGame(game); setShowGameCenter(false); }} />
      ...
      <AnimatePresence>{showProfileSheet && selectedUserForProfile && (<UserProfileSheet user={selectedUserForProfile} onClose={() => setShowProfileSheet(false)} isCurrentUser={selectedUserForProfile.id === currentUser.id} onAction={(action) => {
          if (action === 'gift') { onPushState?.(); setSelectedRecipientIds([selectedUserForProfile.id]); setShowGifts(true); setShowProfileSheet(false); }
          if (action === 'message') { onPushState?.(); onOpenPrivateChat(selectedUserForProfile); }
          if (action === 'edit') { onPushState?.(); setShowEditProfileModal(true); }
          if (action === 'cp') { onPushState?.(); onOpenCP(); }
          if (canAdmin) {
            const roomRef = doc(db, 'rooms', initialRoom.id);
            if (action === 'addModerator') updateDoc(roomRef, { moderators: arrayUnion(selectedUserForProfile.id) });
            if (action === 'removeModerator') updateDoc(roomRef, { moderators: arrayRemove(selectedUserForProfile.id) });
            if (action === 'removeMic') { const upd = localSpeakers.filter(s => s.id !== selectedUserForProfile.id); setLocalSpeakers(upd); queueRoomSpeakersUpdate(upd); }
            if (action === 'kickAndBan') { const upd = localSpeakers.filter(s => s.id !== selectedUserForProfile.id); setLocalSpeakers(upd); updateDoc(roomRef, { kickedUsers: arrayUnion(selectedUserForProfile.id), speakers: upd }); }
            if (action === 'resetUserCharm') { const upd = localSpeakers.map(s => s.id === selectedUserForProfile.id ? { ...s, charm: 0 } : s); setLocalSpeakers(upd); queueRoomSpeakersUpdate(upd); }
          }
      }} currentUser={currentUser} allUsers={users} currentRoom={room} />)}</AnimatePresence>
      ...
      <AnimatePresence>{showParticipants && ( <RoomParticipantsModal isOpen={showParticipants} onClose={() => setShowParticipants(false)} participants={roomParticipants} onSelectUser={(u) => { onPushState?.(); setSelectedUserForProfile(u); setShowProfileSheet(true); }} /> )}</AnimatePresence>
    </div>
  );
};
