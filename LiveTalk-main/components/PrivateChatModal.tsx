...
import { X, Send, User as UserIcon, Shield, Smile, Paperclip, MoreVertical, Trash2 } from 'lucide-react';
import { User } from '../types';
import { db } from '../services/firebase';
import { collection, addDoc, onSnapshot, query, orderBy, serverTimestamp, limit, doc, setDoc, deleteDoc } from 'firebase/firestore';
...
const PrivateChatModal: React.FC<PrivateChatModalProps> = ({ partner, currentUser, onClose }) => {
  ...
  const handleDeleteMessage = async (msgId: string) => {
    ...
    try {
      await deleteDoc(doc(db, 'private_chats', chatId, 'messages', msgId));
          ...
          {loading ? (
            ...
            messages.map((msg) => {
              ...
              return (
                ...
                >
                  <div className={`relative max-w-[85%] px-5 py-3 rounded-2xl text-xs md:text-sm shadow-xl flex items-center gap-3 ${
                    isMine 
                      ? 'bg-gradient-to-r from-amber-500 to-orange-600 text-black rounded-tr-none font-black' 
                      : 'bg-slate-800/80 backdrop-blur text-white rounded-tl-none border border-white/5 font-bold'
                  }`}>
                    ...
                    <button 
                      ...
                      className={`p-1 rounded-lg transition-colors ${isMine ? 'hover:bg-black/10 text-black/50' : 'hover:bg-white/10 text-white/50'}`}
                    >
                    ...
                    <AnimatePresence>
                      {isMenuOpen && (
                        <motion.div 
                          ...
                          className={`absolute bottom-full mb-2 z-20 bg-slate-900 border border-white/10 rounded-xl p-1 shadow-2xl ${isMine ? 'right-0' : 'left-0'}`}
                        >
                          <button 
                            onClick={() => handleDeleteMessage(msg.id)}
                            className="flex items-center gap-2 px-4 py-2 text-red-500 hover:bg-red-500/10 rounded-lg text-[10px] font-black whitespace-nowrap"
                          >
                            <Trash2 size={12} /> حذف الرسالة
                          </button>
                        </motion.div>
                      )}
                    </AnimatePresence>
