  ...
  const idDisplay = useMemo(() => {
     ...
     return (
        <div className="relative inline-flex items-center justify-center">
           {badge ? (
             <div className="relative flex items-center justify-center h-7 min-w-[85px] group">
               <img 
                 src={badge} 
                 className="absolute inset-0 w-full h-full object-contain pointer-events-none z-0 drop-shadow-[0_2px_4px_rgba(0,0,0,0.5)]" 
                 alt="ID Badge"
               />
               {/* تم حذف pl-3 لتوسيط النص */}
               <span className="relative z-10 text-white font-black text-[9px] drop-shadow-[0_1px_2px_rgba(0,0,0,0.8)] whitespace-nowrap tracking-tight">
                  ID: {displayId}
               </span>
             </div>
           ) : (
